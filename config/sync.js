/**
 * Updates configuration variables of your app that is hosted on Heroku.
 * Important: You must first log into heroku in the command line.
 *
 * Run:
 * node ./config/sync .env.template heroku-app-name isFirstSync
 */
...
/**
 * Updates your heroku configuration variables for an app from a .env file in the config folder.
 *
 * Arguments: accepts both command line arguments and parameters
 * Ex: node ./config/sync .env.template heroku-app-name first
 *
 * @envFile (STRING - REQUIRED): the path of the .env file [.env.template]
 * @appName (STRING - REQUIRED): the name of the heroku app you want to update the configuration variables for
 * @isFirstSync (STRING BOOLEAN - OPTIONAL): if any value is passed in, then we know this is the first time we are setting configuration variables, otherwise if not value is passed in, we reset and override current configuration variables.
 */
