...
function checkPasswords(password1, password2, pwMinLen) {
  ...
  if (password1 !== password2) return 'The passwords you entered do not match.';

  // check minimum length of the password
  if (password1.length < pwMinLen) return 'Password length must be ' + pwMinLen + ' characters or greater.';

  // check if password has ' ' characters (white space)
  if (password1.indexOf(' ') >= 0) return 'Password cannot contain white spaces.';
