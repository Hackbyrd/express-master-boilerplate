...
// ENV variables
const { NODE_ENV, SENDGRID_KEY } = process.env;
 ...
 *   @args: (OBJECT) The variables to pass to the email template (refer to args in template using -arg- syntax) MUST BE ALL STRING VALUES
 * }
 *
...
function mail({ from, name, subject, template, tos, ccs, bccs, args }, callback) {
  ...
  if (extension) return callback(new Error(`Cannot send an email from a "${extension}" domain because it violates the DMARC policy.`));

  // validate tos
  if (tos === null || tos === undefined || tos.length === 0) return callback(new Error('Email must be sent to at least one recipient.'));

  // set ccs to empty array if not specified
  if (ccs === null || ccs === undefined) ccs = [];

  // set bccs to empty array if not specified
  if (bccs === null || bccs === undefined) bccs = [];
  ...
  // convert file
  ejs.renderFile(path.join(__dirname, `../mailers/${template}/index.ejs`), args, (err, rawHtml) => {
...
function dmarc(from) {
  ...
  for (let i = 0; i < BLOCKED_EMAILS.length; i++) {
    ...
    if (from.indexOf(extension) >= 0) return extension;
  }
